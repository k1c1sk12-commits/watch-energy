...
check("pool covers all 5 energies on both slots", balErrors.length === 0, balErrors.join("; "));
...
check("at least 8 distinct watches surface", sorted.length >= 8, `${sorted.length}`);
const topShare = sorted[0][1] / total;
check("no single watch dominates >40%", topShare <= 0.4, `${(topShare * 100).toFixed(1)}%`);
...
for (const [dob, v] of [
  ...
  console.log(`\n  [${v}] ${r.watch.brand} ${r.watch.model} — ${r.matchPercent}% · TOP ${r.rarity}%`);
  console.log(`  ${r.reason}`);
  console.log(`  traits: ${r.traits.join(" · ")}`);
}
